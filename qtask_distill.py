...
from transformer.modeling import TinyBertForSequenceClassification, BertConfig
...
def main():
    ...
    # NOTE(raoul): Here, we load the dynamically quantized model differently than by default.
    # This is an AI-generated suggestion to get the evaluation to recognize the `bert.encoder` layers
    # of the quantized model.
    if "dynamic" in args.student_model or "quantized" in args.student_model:
        logger.info("***** Loading (Dynamic) Quantized Student Model *****")

        config_file = os.path.join(args.student_model, CONFIG_NAME)
        config = BertConfig.from_json_file(config_file)

        student_model = TinyBertForSequenceClassification(config, num_labels=num_labels)
        
        # NOTE(raoul): Suggested by Gemini to set only the BERT encoder layers in order
        # not to touch and destroy anything else (like `fit_dense` layer).
        student_model.bert.encoder = torch.quantization.quantize_dynamic(
            student_model.bert.encoder, {torch.nn.Linear}, dtype=torch.qint8
        )
        
        # Load the quantized weights into the prepared structure
        model_file = os.path.join(args.student_model, "pytorch_model.bin")
