...
def convert_mask_to_dict(_mask):
    ...
    for l in range(num_layers):
        _d[l] = torch.arange(num_units)[(_mask[l].cpu() < 1e-5)].tolist()
    return _d
...
def remove_paddings(_value, att_mask):
    """
    Remove padded positions from _value according to att_mask.

    Args:
        _value: Tensor of shape [batch, seq_len, ...]
        att_mask: Bool or 0/1 mask of shape [batch, seq_len]

    Returns:
        Tensor with padding positions removed.
    """
    # ensure mask is on same device as _value
    att_mask = att_mask.to(_value.device)
